...
export const db = {
    ...
    saveInvoice: async (id, data) => {
        try {
            ...
            // 1. Fetch existing for Audit Diff
            ...
            // 2. Calculate Diffs & Generate Logs
            const logsToCreate = [];
            ...
            if (existing) {
                ...
                if (data.status && data.status !== existing.status) {
                    logsToCreate.push({
                        invoice_id: id, username: 'System', action: 'STATUS_CHANGE',
                        details: `Status changed from ${existing.status} to ${data.status}`
                    });
                }
                if (data.amount && parseFloat(data.amount) !== parseFloat(existing.amount)) {
                    logsToCreate.push({
                        invoice_id: id, username: 'System', action: 'UPDATE',
                        details: `Amount updated from ${existing.amount} to ${data.amount}`
                    });
                }
                if (data.vendorName && data.vendorName !== existing.vendorName) {
                    logsToCreate.push({
                        invoice_id: id, username: 'System', action: 'UPDATE',
                        details: `Vendor updated from ${existing.vendorName} to ${data.vendorName}`
                    });
                }
                if (data.poNumber && data.poNumber !== existing.poNumber) {
                    logsToCreate.push({
                        invoice_id: id, username: 'System', action: 'UPDATE',
                        details: `PO Number updated from ${existing.poNumber} to ${data.poNumber}`
                    });
                }
            } else {
                logsToCreate.push({
                    invoice_id: id, username: 'System', action: 'CREATE',
            ...
            if (logsToCreate.length > 0) {
                await AuditTrail.insertMany(logsToCreate);
            }
