...
export const sendStatusNotification = async (invoice, nextStatus) => {
    ...
    if (!apiKey) {
        if (!sendStatusNotification._warnedNoSendGrid) {
            sendStatusNotification._warnedNoSendGrid = true;
            console.warn("[Notification] SENDGRID_API_KEY not set. Email dispatch skipped. Set in .env.local to enable.");
        }
        return;
    }
